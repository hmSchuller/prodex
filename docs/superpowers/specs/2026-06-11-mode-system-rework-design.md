# Mode System Rework: Fast / Standard / Pro
...
- Replace `build` and `plan` agents with `fast`, `standard`, `pro` primary modes
...
|------|----------|---------|-------------|
| Fast | opencode-go/mimo-v2.5 | Yes | Cheapest/quickest, for simple tasks |
...
| General | Inherits current mode | Smart worker: Fast→mimo-v2.5, Standard→mimo-v2.5-pro, Pro→gpt-5.5 |
...
Mirror the same changes: replace build/plan with fast/standard/pro, add runner, update explore/general.
...
Update every built-in fallback from `"build"` to `"fast"`:
...
// packages/core/src/agent.ts
export const defaultID = ID.make("fast")

// packages/core/src/agent.ts selectedDefault()
const fast = selectable(data.agents.get(ID.make("fast")))
if (fast) return fast
...
Update `--agent` flag validation to accept `fast`, `standard`, `pro` instead of `build`, `plan`.
...
1. User selects mode (Fast/Standard/Pro) via TUI selector or `--agent` CLI flag
...
4. Default mode is Fast if not configured
...
- Users with `default_agent: "build"` in config should change to `default_agent: "fast"`
